...
- 숫자를 한글로 변환
- 총 195개의 토시 처리 가능 (변환 필요 토시 162개 + 불변 토시 33개)
...
| identifier.rs | 277 | 토시 종류 판별 및 변환 방식 결정 |
| tossi.rs | 262 | 처리 가능한 토시 목록 정의 (195개) |
| transfer.rs | 1246 | 토시 변환 로직 (가장 큰 모듈) |
...
- 1246 라인으로 전체 코드의 약 40% 차지
- 51개 토시별 변환 상수 정의 (3-튜플: 병기, 무받침, 유받침)
- 6가지 변환 방식: Blank, RiEulAndBlank, OnlyKa, LastJamoNieun, LastJamoRieul, Nothing

### identifier (토시 분류)

- `TossiKind` enum: 51개 토시 종류 + Others
...
| 의존성 | 최소 (외부 의존 없음) |
| 기능 범위 | 포괄적 (195개 토시, 51종 변환, 특수 케이스 처리) |
