...
- 숫자를 한글로 변환
- 총 177개의 토시 처리 가능 (변환 필요 토시 144개 + 불변 토시 33개)
...
| identifier.rs | 259 | 토시 종류 판별 및 변환 방식 결정 |
| tossi.rs | 244 | 처리 가능한 토시 목록 정의 (177개) |
| transfer.rs | 1228 | 토시 변환 로직 (가장 큰 모듈) |
...
- 1228 라인으로 전체 코드의 약 40% 차지
- 45개 토시별 변환 상수 정의 (3-튜플: 병기, 무받침, 유받침)
- 6가지 변환 방식: Blank, RiEulAndBlank, OnlyKa, LastJamoNieun, LastJamoRieul, Nothing

### identifier (토시 분류)

- `TossiKind` enum: 45개 토시 종류 + Others
...
| 의존성 | 최소 (외부 의존 없음) |
| 기능 범위 | 포괄적 (177개 토시, 45종 변환, 특수 케이스 처리) |
