...
file_name_transfer_rs = os.path.join(temp_dir, "src", "transfer.rs")

if os.path.isfile(file_name_transfer_rs):
    with open(file_name_transfer_rs, encoding="utf-8") as f:
        read_transfer_rs = f.read()
else:
    print(f"오류: '{file_name_transfer_rs}' 파일을 찾을 수 없습니다.", file=sys.stderr)
    sys.exit(1)
...
# tossi.rs 에 `TOSSI_LIST`이라는 이름으로 들어 있습니다.
# 이것을 뽑아내면 현재 처리할 수 있는 전체 목록이 나오게 됩니다.
file_name_tossi_rs = os.path.join(temp_dir, "src", "tossi.rs")

if os.path.isfile(file_name_tossi_rs):
    with open(file_name_tossi_rs, encoding="utf-8") as f:
        read_tossi_rs = f.read()
else:
    print(f"오류: '{file_name_tossi_rs}' 파일을 찾을 수 없습니다.", file=sys.stderr)
    sys.exit(1)

# tossi.rs 코드에서 엔터 문자를 제거한다.
read_tossi_rs = read_tossi_rs.replace("\n", "")
# print(f"full: {read_tossi_rs}")
# 앞의 `const TOSSI_LIST` 을 뽑기 위해 `];`으로 자른다.
# 자르고 나면 맨 처음 부분 `[0]`에 우리가 뽑을 것이 들어가게 된다.
# 참고로 이렇게 자르면 `]`만 없어지기 때문에, 남은 '['을 제거해야 한다.
temp_str = read_tossi_rs.split("];")
...
# 전체 목록에서 붙일 단어에 따라 변환하는 토시들 목록을 뺍니다.
...
print(
    " 그렇기 때문에 이 라이브러리가 이 두 가지를 파악하는 것 또한 중요한 기능 중 하나입니다."
)
print(
...
print("")
print(
    '참고로 "어서", "었다", "에요"는 `(이)어서`, `(이)었다`, `(이)에요`와 관련이 있지만,'
)
print(
    " 이것들을 독자적으로 입력하면 앞 단어에 상관없이 변하지 않기 때문에 이 목록에 포함되어 있습니다."
)
print(
    " 변환이 일어나는 것은 `(이)어서`처럼 괄호 형태로 입력했을 때뿐입니다."
)
print("")
...
print(
    " 동일한 토시이지만 괄호가 다양하게 들어가 있어 같은 토시를 중복 것들을 처리하고 있기 때문이 목록은"
)
...
json_output_path = os.path.join(temp_dir, "docs", "total_tossi.json")
