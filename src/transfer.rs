...
/// ## TossiKind에 맞는 토시 변형 후보를 반환하는 함수
///
/// 토시 종류에 맞는 변형 후보 3개(병기, 받침없음, 받침있음)를 반환합니다.
/// `tossi()`와 `split_word_tossi()` 함수에서 공통으로 사용합니다.
...
pub fn tossi(word: &str, tossi: Tossi) -> String {
    let tossi_variants = get_variants(&tossi.kind);
...
mod tests {
    ...
    fn _when_blank_ya() {
        ...
        let temp = "사람";
        let result = "아";
        assert_eq!(result, when_blank(temp, YA));
        let temp = "친구들";
        let result = "아";
        assert_eq!(result, when_blank(temp, YA));
        // 마지막 받침이 없는 경우
