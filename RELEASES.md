...
현재 `TOSSI_LIST`에 등록된 토시는 총 192개이며, 그 중 변환이 필요 없는 토시는 33개, 변환 처리 가능한 토시는 159개(괄호 포함 59개, 괄호 없는 100개)입니다. 실제 `TossiKind`로 매핑되어 변환을 처리하는 토시 종류는 총 50종류이며, 변환 방식에 따라 다음과 같이 분류됩니다.
