...
현재 `TOSSI_LIST`에 등록된 토시는 총 174개이며, 그 중 변환이 필요 없는 토시는 33개, 변환 처리 가능한 토시는 141개(괄호 포함 53개, 괄호 없는 88개)입니다. 실제 `TossiKind`로 매핑되어 변환을 처리하는 토시 종류는 총 44종류이며, 변환 방식에 따라 다음과 같이 분류됩니다.
...
- "로도/으로도", "로만/으로만", "로는/으로는" 조사 추가 (RiEulAndBlank 패턴)
- "더러" 안 변하는 토시 추가
