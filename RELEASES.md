...
현재 `TOSSI_LIST`에 등록된 토시는 총 164개이며, 그 중 변환이 필요 없는 토시는 32개, 변환 처리 가능한 토시는 132개(괄호 포함 50개, 괄호 없는 82개)입니다. 실제 `TossiKind`로 매핑되어 변환을 처리하는 토시 종류는 총 41종류이며, 변환 방식에 따라 다음과 같이 분류됩니다.
